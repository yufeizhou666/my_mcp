...
async function getWindowsMemory(): Promise<{ total: number; used: number; percent: number }> {
  try {
    const { stdout } = await execAsync(
...
async function getWindowsDisk(): Promise<{ total: number; used: number; percent: number }> {
  try {
    const { stdout } = await execAsync(
...
async function getLinuxMemory(): Promise<{ total: number; used: number; percent: number }> {
  try {
...
async function getLinuxDisk(): Promise<{ total: number; used: number; percent: number }> {
  try {
...
export async function getSystemMetrics(): Promise<SystemMetrics> {
  ...
  let cpu: { usage: number; cores: number } = { usage: 0, cores: os.cpus().length };
